...
export class NotificationSection implements OnInit {
  ...
  async clear() {
    await firstValueFrom(this.notificationService.clear());
    this.notifications = []
  }
  ...
  async onItemDeleted(idx: number) {
    await firstValueFrom(this.notificationService.delete(this.notifications[idx].id));
    this.notifications = [...this.notifications.slice(0, idx), ...this.notifications.slice(idx + 1)]
  }
